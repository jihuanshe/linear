...
- `issue list` sorts by priority by default — override via `--sort` (valid values: `manual`, `priority`), the `issue_sort` config option, or the `LINEAR_ISSUE_SORT` env var. `--team` is optional when a default team is configured; use it to override that default.
- `--no-pager` is supported on `issue mine`/`issue list` and `issue query`, but not on unrelated list commands such as `project list`.
