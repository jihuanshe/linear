...
Linear CLI 从命令选项、环境变量、项目 TOML 和用户 TOML 解析配置。认证凭据的存储与工作区选择另见[认证与工作区凭据](authentication.md)。
...
命令会交互选择工作区、团队和 Issue 排序，然后写入 Git 根目录；不在 Git 仓库中时则写入当前目录：
...
高优先级值在选中后由对应设置的解析逻辑校验。无效的 `issue_sort` 会明确报错；其他设置的无效值按对应命令的解析与默认规则处理，不能据此假定所有设置都会报错。

API 密钥和 `--workspace` 的身份选择有独立的安全优先级，不套用这份普通设置顺序；见[认证与工作区凭据](authentication.md)。
...
CLI 还会读取当前目录的 `.env`；不存在时再读取 Git 根目录的 `.env`。只接受 `LINEAR_`、`GH_` 和 `GITHUB_` 前缀，且不会覆盖当前进程已经设置的环境变量。不要把 API 密钥提交进项目配置或 `.env`。

## 设置

| TOML key                    | 环境变量                           | 取值与作用                                                                                           |
| --------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `workspace`                 | `LINEAR_WORKSPACE`                 | 默认工作区短名；从已保存凭据中选择对应 API 密钥                                                      |
| `team_id`                   | `LINEAR_TEAM_ID`                   | 默认团队 key，例如 `ENG`                                                                             |
| `issue_sort`                | `LINEAR_ISSUE_SORT`                | `priority`（默认）或 `manual`                                                                        |
| `issue_create_ask_project`  | `LINEAR_ISSUE_CREATE_ASK_PROJECT`  | 创建 Issue 时是否询问所属项目                                                                        |
...
| `auto_download_attachments` | `LINEAR_AUTO_DOWNLOAD_ATTACHMENTS` | 以非 JSON 输出查看 Issue 时是否下载侧栏文件附件（Attachment）；默认开启，并受 `download_images` 控制 |
| `attachment_dir`            | `LINEAR_ATTACHMENT_DIR`            | 侧栏附件下载目录；默认使用系统临时目录下的 `linear-cli-attachments`                                  |
...
| `LINEAR_GRAPHQL_ENDPOINT`  | 覆盖 GraphQL 端点，用于受控代理                  |
| `LINEAR_DEBUG=1`           | 在 stderr 输出底层错误和堆栈                     |
| `NO_COLOR=1`               | 禁止颜色、加载动画和 OSC-8 链接等 TTY 样式       |

每个命令支持的显式选项和运行时默认值以 `linear <command> --help` 为准；无人值守输出契约见 `linear guide automation`。
