...
发现问题的系统不自动拥有修复。日志或分析只能证明现象；修复责任和验收应落在拥有权威事实的系统。
...
写入须确认认证和目标 workspace，可用 `auth whoami`；apply 自带身份核对。用户明确要求按给定内容创建或修改已构成授权，无需重复确认。未决结论会改变本次目标、责任、访问范围或业务结果，或涉及未授权的批量覆盖、公开上传或敏感材料处理时，须展示草稿或变更摘要并取得确认；不改变这些决定的措辞细节直接处理。调查、查看和起草请求不授权写入。
...
多个执行项且操作和字段均在支持范围内时用 [delivery manifest](issue-delivery.md)，单项或超出 manifest 范围的操作用对应专用命令；跨命令编排见 [automation](automation.md)。已有授权且内容明确时直接 apply；plan 仅用于可选预览，不增加审批步骤。
