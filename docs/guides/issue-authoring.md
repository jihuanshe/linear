...
Issue 参数支持原生 UUID 和完整编号（如 ENG-123）；UUID 会解析为当前编号，继续支持已有数字和 VCS 上下文捷径。`issue view --json` 返回 `id` 与 `identifier`，可把创建或查询结果中的 ID 直接传给 view、update、comment、attach、link 等命令。

`--state` 与标签的设置、增删参数支持 UUID；也保留原有名称／状态类型用法。UUID 只按 ID 匹配，不会退回同名搜索；状态必须属于目标团队，标签必须属于该团队或工作区。标签输入用于 create 的交互模式时，无效 UUID 也不会弹出名称模糊匹配。

使用旧编号更新状态、标签、周期或项目时，以工单当前团队解析。显式 `--team` 使用目标团队；`issue start <UUID或编号>` 无需默认团队，使用远端当前编号准备本地 VCS 工作，并更新当前团队的状态。

`issue apply` 的清单格式与恢复记录保持不变：manifest 的 `identifier` 和 relation 目标仍使用完整编号，`set.state`、`set.labels` 可使用上述 ID。
