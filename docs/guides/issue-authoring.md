...
写入前先用 `auth whoami` 确认认证与目标 workspace，不要等创建失败才发现。创建或实质性改写前，向用户展示拟定标题、正文、推断的 team/labels/assignee、文件与链接清单、显式假设和缺失证据；沉默不算确认。创建因权限或 workspace 错误失败时，保留已确认的草稿，恢复访问后再提交，不重写内容。
