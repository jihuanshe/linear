...
正文说明具体情景、遇到的问题、已确认事实和要解决的工作，让没有前情的同事也能接手。信息不足时指出具体缺口，区分事实和推测。按内容需要使用自然段、列表或图；预期行为写到可以核验的程度，例如“横屏后按钮应保持可点击”。
...
讨论形成结论后，把仍适用的事实和决定纳入正文；需要后续处理的工作保留明确去向。原评论保存历史，问题得到回答或有清楚承接后，用 `issue comment resolve <评论 ID>` 收束，可用 `--resolving-comment <回复 ID>` 关联结论。仍缺证据或决定时保持开放，判断改变后可以 `unresolve`。
