...
| 普通对象创建 | 解析引用；检查 `success` 和返回对象的稳定 ID | 没有旧值依据；重复调用可能创建多个对象 |
| Issue `--add-label` / `--remove-label` | 发送上游增量字段，不把集合读回后整体覆盖 | 不走 `--base-file`；不是通用的并发事务或幂等保证 |
| 评论、附件和关联创建 | 检查输入；用返回对象确认 ID，并核对返回对象的父对象 | 不是幂等写入；重复调用可能产生重复对象 |
| 关系创建 | 检查两端 ID；已读到同类型、同方向关系时跳过写入 | 已有关系不会再次写入；两个并发首次调用仍可能重复创建 |
| 删除和归档 | 部分命令先读状态、要求确认或核对返回 ID | 预读、确认和成功回执都不是锁；多数路径不做删除后的读回 |
| 上传文件 | 先捕获并校验本地字节；保存上传回执和已确认的效果 | 签发上传许可或 PUT 失败不能证明远端效果；不要盲目重传 |
| resolve／unresolve | 读取当前状态并处理已满足目标的情况；成功保留 mutation 回执 | 预读不是锁；两个执行者可能基于不同状态作出决定 |
