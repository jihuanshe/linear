...
普通更新直接使用专用命令和保存的原始依据，见 [automation](automation.md)。需要组合多项写入并记录进度时使用 [issue-delivery](issue-delivery.md)；Git/Jujutsu、GitHub 和组织治理流程由 [recipes](../../recipes/README.md) 提供可运行示例。

以下名称相近，但操作不同：

- `project update` 修改项目，`project-update create/list` 发布或读取项目进展；`initiative update` 与 `initiative-update create/list` 同理。
- Project `description` 是短简介，长正文属于 overview。创建时用 `project create --content-file`；`project update` 尚不支持 overview，更新它属于 `linear api --unprotected` 的长尾写入。
