...
多行正文优先使用目标命令的文件输入，具体参数见该命令的 `--help`。替换前保存带身份的原始读取并传 `--base-file`；比较与显式无保护入口见 [automation](automation.md)。交付清单中的描述和评论遵循相同规则，执行与恢复见 [issue-delivery](issue-delivery.md)。
