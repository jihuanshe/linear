...
- Made update manifests fail closed: every replacement field requires its last-read `base`, including explicit null and empty-collection values. By default, a fresh apply preflights every update target before the first mutation and returns structured zero-write failures; `--continue-on-failure` instead checks each Issue as it goes. Both paths read each Issue immediately before its own write, preventing known batch conflicts from causing avoidable partial delivery without claiming a server-side lock or transaction.
