...
- Made update manifests fail closed: every replacement field requires its last-read `base`, including explicit null and empty-collection values. A fresh apply preflights every update target before the first mutation, then re-reads each Issue immediately before its own write; this prevents known batch conflicts from causing avoidable partial delivery without claiming a server-side lock or transaction.
