...
- Made a manifest with multiple issues the batch form of the same protocol: a checkpoint beside the manifest skips confirmed successes on resume, refuses resumes after structural drift or unresolved unknown outcomes, and `--continue-on-failure` collects failures without stopping.
