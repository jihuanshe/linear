...
- Made update manifests fail closed: every replacement field requires its last-read `base`, including explicit null and empty-collection values. Apply reads each Issue immediately before its own write and refuses fields that changed since `base`; batches remain sequential, with checkpointed partial delivery and explicit stop/continue behavior rather than a batch lock or transaction claim.
