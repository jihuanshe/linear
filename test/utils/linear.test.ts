...
for (const outcome of ["found", "missing", "error"] as const) {
  Deno.test(`getIssueIdentifier resolves a UUID without shorthand fallback: ${outcome}`, async () => {
    const id = "abcdef01-2345-4678-9abc-def012345678"
    const { server, cleanup } = await setupMockLinearServer([{
      queryName: "GetIssueIdentifierById",
      variables: { id },
      response: outcome === "error"
        ? { errors: [{ message: "Identity lookup failed" }] }
        : {
          data: {
            issue: outcome === "found" ? { identifier: "NEW-42" } : null,
          },
        },
    }], { LINEAR_TEAM_ID: "OLD" })
    try {
      if (outcome === "found") {
        assertEquals(await getIssueIdentifier(id.toUpperCase()), "NEW-42")
      } else {await assertRejects(
          () => getIssueIdentifier(id),
          Error,
          outcome === "missing" ? "Issue not found" : "Identity lookup failed",
        )}
...
Deno.test("resolveWorkflowState uses exact UUID identity and no UUID-shaped name fallback", () => {
  const wanted = "abcdef01-2345-4678-9abc-def012345678",
    other = "abcdef02-2345-4678-9abc-def012345678"
  const states = [{ id: other, name: wanted, type: "started", position: 1 }, {
    id: wanted,
    name: "Working",
    type: "started",
    position: 2,
  }]
  assertEquals(resolveWorkflowState(states, wanted.toUpperCase())?.id, wanted)
  assertEquals(resolveWorkflowState(states.slice(0, 1), wanted), undefined)
})
