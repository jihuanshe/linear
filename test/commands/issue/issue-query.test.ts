...
for (const named of [false, true]) {
  Deno.test(`Issue Query Command - search applies a ${named ? "named" : "UUID"} milestone`, async () => {
    const milestoneId = "abcdef01-2345-4678-9abc-def012345678"
    const projectId = "abcdef02-2345-4678-9abc-def012345678"
    const { server, cleanup } = await setupMockLinearServer([
      {
        ...
        response: {
          data: {
            project: {
              projectMilestones: {
                nodes: [{ id: milestoneId, name: "Release candidate" }],
              },
            },
          },
        },
    ...
    const logs: string[] = []
    const logStub = stub(console, "log", (value: string) => logs.push(value))
    try {
      await queryCommand.parse([
        "--all-teams",
        "--search",
        "evidence",
        "--milestone",
        named ? "Release candidate" : milestoneId,
        ...(named ? ["--project", projectId] : []),
        "--json",
      ])
      assertEquals(server.graphqlRequests.length, named ? 2 : 1)
      assertEquals(server.graphqlRequests.at(-1)?.variables, {
        term: "evidence",
        first: 50,
        filter: {
          ...(named ? { project: { id: { eq: projectId } } } : {}),
...
for (const failure of ["empty", "cycle"]) {
  Deno.test(`Issue Query Command - exact URL rejects ${failure} comment pagination without partial stdout`, async () => {
    const { server, cleanup } = await setupMockLinearServer([
      {
        queryName: "GetIssuesForQuery",
        response: {
      ...
      ...[
        { after: "a", endCursor: "b" },
        { after: "b", endCursor: "a" },
        { after: undefined, endCursor: failure === "empty" ? "" : "a" },
      ].map(({ after, endCursor }) => ({
        queryName: "GetIssueCommentsForUrlLookup",
        ...
        response: {
          data: {
            issue: {
              comments: {
                nodes: [{ body: "https://example.com/evidence" }],
                pageInfo: { hasNextPage: true, endCursor },
              },
            },
          },
    ...
    try {
      ...
      assertStringIncludes(
        ...
        "Incomplete comments for issue-1 pagination: empty or repeated cursor",
      )
      assertEquals(server.graphqlRequests.length, failure === "empty" ? 2 : 4)
    } finally {
